...
class QualityMetrics:
    ...
    def detect_green_channel_shift(self, image: np.ndarray) -> float:
        ...
        Measures correlation between green channel and a lightweight edge mask.
        ...
        # Use finite-difference gradients instead of the heavier Sobel pass to keep
        # the metric fast on large datasets while preserving the same intent.
        gray = cv2.cvtColor(image, cv2.COLOR_RGB2GRAY).astype(np.float64)
        ...
        max_grad = float(np.max(gradient_magnitude)) if gradient_magnitude.size > 0 else 0.0
        if max_grad > 0:
            gradient_mask = gradient_magnitude / max_grad
        else:
        ...
        return float(green_shift_score)
    ...
    def compute_sharpness_score(self, image: np.ndarray) -> float:
        """
        Compute a distinct sharpness-like metric from image edge density.
        This is intentionally different from blur: blur is low when the image is fuzzy,
        while sharpness is high when the image is over-sharpened.
        """
        gray = cv2.cvtColor(image, cv2.COLOR_RGB2GRAY).astype(np.float64)
            ...
            for img_path in image_paths:
                try:
                    ...
                    if is_corrupted:
                        ...
                        self.sharpness_scores.append(0.0)
                    ...
                    sharpness = self.compute_sharpness_score(image)
    ...
    def get_statistics(self) -> Dict[str, Dict[str, float]]:
        ...
        # Sharpness is measured separately and intentionally differs from blur.
        if len(self.sharpness_scores) > 0:
            stats['sharpness'] = {
                'mean': np.mean(self.sharpness_scores),
                'std': np.std(self.sharpness_scores),
                'min': np.min(self.sharpness_scores),
                'max': np.max(self.sharpness_scores),
                'median': np.median(self.sharpness_scores)
            }
